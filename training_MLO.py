...
import model_b_modified.model_MLO as model
import model_b_modified.config_MLO as config
import numpy as np
    ...
    for i in range(len(list_of_patient)):
        for file in glob.glob(config.folder_path["train_dataset_glob"]):
            if str(str(list_of_patient[i][0]).split("_")[-1]) in str(file):
                ...
                temp_path_RMLO = config.folder_path["train_dataset"] + "_" + key_word + "_" + "RIGHT" + "_" + "MLO" + ".png"
                temp_path_LMLO = config.folder_path["train_dataset"] + "_" + key_word + "_" + "LEFT" + "_" + "MLO" + ".png"
                ...
                try:
                    temp_path_MLO = cv2.imread(temp_path_RMLO,0)
                except Exception as e:
                    print(e)
                if temp_path_MLO is None:
                    try:
                        temp_path_MLO = cv2.imread(temp_path_LMLO,0)
                        temp_path_MLO = cv2.flip(temp_path_MLO, 1)
                    except Exception as error:
                        print(error)
                try:

                    train_x1_rcc = cv2.resize(temp_path_MLO, (2000, 2600))
...
def test_data():
    file_name = config.file_path["test_xlsx"]
    list_of_patient = []
    df = pd.read_excel(io=file_name)
    patient_id = df['patient_id'].tolist()
    target_value = df["assessment"].tolist()
    ...
    for i in range(len(list_of_patient)):
        for file in glob.glob(config.folder_path["test_dataset_glob"]):
            if str(str(list_of_patient[i][0]).split("_")[-1]) in str(file):
                key_word = str(str(list_of_patient[i][0]).split("_")[-1])
                target_element = str(list_of_patient[i][1])
                temp_path_RMLO = config.folder_path["test_dataset"] + "_" + key_word + "_" + "RIGHT" + "_" + "MLO" + ".png"
                temp_path_LMLO = config.folder_path["test_dataset"] + "_" + key_word + "_" + "LEFT" + "_" + "MLO" + ".png"
                #print(temp_path_CC, temp_path_MLO, target_element)
                # image = cv2.imread(temp_path_MLO,0)
                list_patient_test_data.append(str(list_of_patient[i][0]).split("_")[-1])
                try:
                    train_x1_rcc = cv2.imread(temp_path_RMLO,0)
                except Exception as error:
                    print(error)
                if train_x1_rcc is None:
                    try:
                        train_x1_rcc = cv2.imread(temp_path_LMLO,0)
...
optimizer = tf.train.AdamOptimizer(learning_rate=1e-5, beta1=0.99, epsilon=0.1)
